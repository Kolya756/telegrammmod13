...
private func SGProControllerEntries(presentationData: PresentationData) -> [SGProControllerEntry] {
    ...
    // MARK: Symonagram — App Icon selection removed: the alternate icons are Telegram-branded
    // and their assets are not bundled after our icon rebrand, so they neither preview nor apply.
...
public func sgProController(context: AccountContext) -> ViewController {
    ...
    |> map { presentationData, _ ->  (ItemListControllerState, (ItemListNodeState, Any)) in
        ...
        let controllerState = ItemListControllerState(presentationData: ItemListPresentationData(presentationData), title: .text("Symonagram Pro"), leftNavigationButton: nil, rightNavigationButton: nil, backNavigationButton: ItemListBackButton(title: presentationData.strings.Common_Back))
